...
public class ORecordSerializerBinaryV0 implements ODocumentSerializer {
  ...
  @Override
  public void partialDeserialize(final ODocument document, final BytesContainer container, final String[] iFields) {
    ...
    for (String field : iFields) {
      ...
      final byte[] fieldBytes = field.getBytes(utf8);
      final int fieldSize = fieldBytes.length;
      int size;
      // search:
      while ((size = OVarIntSerializer.readAsInteger(container)) != 0) {
        if (size == fieldSize && compareFieldName(container, fieldBytes, size)) {
          container.skip(size);

          final int valuePos = readInteger(container);

          final OType type = readOType(container);
          if (valuePos != 0) {
            container.offset = valuePos;
            final Object value = readSingleValue(container, type, document);
            // TODO:This is wrong should not stay here
